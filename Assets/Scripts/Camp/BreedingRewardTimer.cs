...
public class BreedingRewardTimer : MonoBehaviour {
    ...
    private void Update()
    {
        if (_timerIsReady)
        {
            if (!_timerComplete && PlayerPrefs.GetString("_breedingTimer") != "")
            {
    ...
    {
        PlayerPrefs.SetString("_breedingTimer", TimeManager.sharedInstance.GetCurrentTime());
        PlayerPrefs.SetInt("_breedingDate", TimeManager.sharedInstance.GetCurrentDateNow());
        StartCoroutine(CheckTime());
    }
    ...
    {
        if (PlayerPrefs.GetString("_breedingTimer") != "" && PlayerPrefs.GetString("_breedingTimer") != "Standby")
        {
            int _old = PlayerPrefs.GetInt("_breedingDate");
    ...
    {
        _startTime = TimeSpan.Parse(PlayerPrefs.GetString("_breedingTimer"));
    ...
    {
        PlayerPrefs.SetString("_breedingTimer", "Standby");
        StartCoroutine(CheckTime());
